...
class dp4orl(object):
    def __init__(self,config,embeddings):
        ...
        with tf.variable_scope('task1'):
            ...
            out_weights = tf.get_variable(name='out_w', shape= [2*config.hidden_size, n_classes[1]],
                                          dtype=tf.float32,
                                          trainable=True)
            out_biases = tf.get_variable( name='out_b',
                                      ...
                                      trainable=True)
            matricized_unary_scores = tf.matmul(outputs_flat, out_weights) + out_biases

            self.unary_scores = tf.reshape(matricized_unary_scores, [tf.shape(self.sentences)[0], -1, n_classes[1]],
                                           name='unary_scores')
