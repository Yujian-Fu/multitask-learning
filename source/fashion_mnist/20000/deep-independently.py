...
def main(args):#? where is args from?
	...
	with tf.Session(config = config) as sess, tf.summary.FileWriter(
			"./tf_logs/deep-independently/result", #+str(datetime.now().timestamp()),
...
def parse_args(argv):
	...
	parser.add_argument("--sharing_layer_enabled", type=bool, help="Use sharing layeror not", default=False)
