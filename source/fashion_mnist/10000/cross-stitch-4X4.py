...
def load_data():
    ...
    train_X = np.load("train_x.npy")[perm]
    train_y_1 = np.load("train_y_1.npy")[perm]
    train_y_2 = np.load("train_y_2.npy")[perm]
    test_X = np.load("test_x.npy")
    test_y_1 = np.load("test_y_1.npy")
    test_y_2 = np.load("test_y_2.npy")
    train_X = train_X[0:10000,:]
    train_y_1 = train_y_1[0:10000,:]
    train_y_2 = train_y_2[0:10000,:]
    test_X = test_X[0:10000,:]
    test_y_1 = test_y_1[0:10000,:]
    test_y_2 = test_y_2[0:10000,:]
...
def combination21(input1, input2):
	combination1 = tf.Variable(0.5, name = "combination1")
	combination2 = tf.Variable(0.5, name = "combination2")
	output1 = tf.multiply(combination1, input1)
	output2 = tf.multiply(combination2, input2)
	output = output1 + output2
	return output

def combination22(input1, input2):
...
def main(args):#输入的数据？*
	...
	#括号里面所有的变量都是叫做network
        with contrib.framework.arg_scope(
            ...
            with tf.variable_scope("combination_1"):
            	combination1_1, combination1_2, combination1_3, combination1_4 = combination44(pool1_1, pool1_2, pool1_3, pool1_4)


            # (?, 14, 14, 32) -> (?, 14, 14, 64)
            conv2_1 = slim.layers.conv2d(combination1_1, 64, kernel_size=[3, 3], scope="conv2_1")
            conv2_2 = slim.layers.conv2d(combination1_1, 64, kernel_size=[3, 3], scope="conv2_2")
            conv2_3 = slim.layers.conv2d(combination1_2, 64, kernel_size=[3, 3], scope="conv2_3")
            conv2_4 = slim.layers.conv2d(combination1_2, 64, kernel_size=[3, 3], scope="conv2_4")
            ...
            #第二层和第二层的cross_stitch单元
            with tf.variable_scope("combination_2"):
                combination2_1 = combination41(pool2_1, pool2_2, pool2_3, pool2_4)
                combination2_2 = combination41(pool2_1, pool2_2, pool2_3, pool2_4)

       
            # (?, 7, 7, 64) -> (?, 3136) -> -> (?, 1024)
            with tf.variable_scope("fc_3_1"):#构建全连接层1
                flatten_1 = contrib.layers.flatten(combination2_1)
                print("fully")
                print(flatten_1.shape)
                fc_3_1 = contrib.layers.fully_connected(flatten_1, 1024)
            with tf.variable_scope("fc_3_2"):#构建全连接层2
                flatten_2 = contrib.layers.flatten(combination2_2)
                fc_3_2 = contrib.layers.fully_connected(flatten_2, 1024)

			#全连接层之后再次接一个combination单元
            with tf.variable_scope("combination_3"):
                combination_fc_3_1, combination_fc_3_2 = combination22(fc_3_1, fc_3_2)


            dropout_1 = contrib.layers.dropout(combination_fc_3_1, keep_prob=keep_prob, is_training=is_training,
                                               scope="dropout_1")
            dropout_2 = contrib.layers.dropout(combination_fc_3_2, keep_prob=keep_prob, is_training=is_training,
                                               scope="dropout_2")
    ...
    with tf.Session(config = config) as sess, tf.summary.FileWriter(
            "./tf_logs/cross-stitch-4X4/result", #+ str(datetime.now().timestamp()),
