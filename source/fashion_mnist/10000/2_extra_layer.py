...
def main(args):#? where is args from?
	...
	with tf.variable_scope("network"):
		with contrib.framework.arg_scope(
			...
			with tf.variable_scope("combination1_1"):
				com_result1 = combination(pool1_1,pool1_2)+pool1_3
				print(com_result1.shape)
			...
			with tf.variable_scope("combination2_1"):
				print(pool2_1.shape)
				print(pool2_3.shape)
				com_result2_1 = combination(pool2_1,pool2_3)


			with tf.variable_scope("combination2_2"):
				com_result2_2 = combination(pool2_2,pool2_3)
	...
	with tf.Session(config = config) as sess, tf.summary.FileWriter(
			"./tf_logs/2_extra_layer/result", #+str(datetime.now().timestamp()),
