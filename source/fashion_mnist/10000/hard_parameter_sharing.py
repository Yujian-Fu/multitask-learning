...
def main(args):#输入的数据？
	...
	#括号里面所有的变量都是叫做network
        with contrib.framework.arg_scope(
            ...
            # (?, 28, 28, 1) -> (?, 28, 28, 32)
            conv1 = slim.layers.conv2d(X, 32, kernel_size=[3, 3], scope="conv1_1")#两层卷积两层pooling层

            # (?, 28, 28, 32) -> (?, 14, 14, 32)
            pool1 = slim.layers.max_pool2d(conv1, kernel_size=[2, 2], stride=2, scope="pool_1_1")
    



            # (?, 14, 14, 32) -> (?, 14, 14, 64)
            conv2 = slim.layers.conv2d(pool1, 64, kernel_size=[3, 3], scope="conv2_1")

            # (?, 14, 14, 64) -> (?, 7, 7, 64)
            pool2 = slim.layers.max_pool2d(conv2, kernel_size=[2, 2], stride=2, scope="pool_2_1")


            # (?, 7, 7, 64) -> (?, 3136) -> -> (?, 1024)
            with tf.variable_scope("fc_3_1"):#构建全连接层1
                flatten_1 = contrib.layers.flatten(pool2)
                print("fully")
                print(flatten_1.shape)
                fc_3_1 = contrib.layers.fully_connected(flatten_1, 1024)
            with tf.variable_scope("fc_3_2"):#构建全连接层2
                flatten_2 = contrib.layers.flatten(pool2)
    ...
    with tf.Session(config = config) as sess, tf.summary.FileWriter(
            "./tf_logs/hard_parameter_sharing/result", graph=tf.get_default_graph()) as f:
