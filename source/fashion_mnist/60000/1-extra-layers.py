...
def load_data():
	...
	train_x = loadImageSet("train-images-ubyte")
	train_y_1 = loadLabelSet("train-labels-ubyte")
	test_x = loadImageSet("test-images-ubyte")
	test_y_1 = loadLabelSet("test-labels-ubyte")
	n_class_1 = 10
	...
	np.save("test_y_1.npy", test_y_1)
	np.save("test_y_2.npy", test_y_2)
...
def main(args):#? where is args from?
	...
	print(train_x.shape, train_y_1.shape, test_x.shape, test_y_1.shape)
	...
	with tf.Session(config = config) as sess, tf.summary.FileWriter(
			"./tf_logs/1_extra_layer/result", #+str(datetime.now().timestamp()),
