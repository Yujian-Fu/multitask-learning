...
def main(args):#? where is args from?
	...
	with tf.Session(config = config) as sess, tf.summary.FileWriter(
			"./cifar_logs/independently/result", #+str(datetime.now().timestamp()),
...
def parse_args(argv):
	...
	parser.add_argument("--cross_stitch_enabled", type=bool, help="Use sharing layeror not", default=False)
