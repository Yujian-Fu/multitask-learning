...
def load_data():
    ...
    cifar_data, cifar_label100, cifar_label20 = rd100.load_CIFAR_batch("./cifar-100-python/train")


    cifar_testdata, cifar_testlabel100, cifar_testlabel20 = rd100.load_CIFAR_batch("./cifar-100-python/test")

    cifar_label20 = to_categorical(cifar_label20, 20)

    cifar_label100 = to_categorical(cifar_label100, 100)

    cifar_testlabel20 = to_categorical(cifar_testlabel20, 20)

    cifar_testlabel100 = to_categorical(cifar_testlabel100, 100)


    return cifar_data, cifar_label100, cifar_label20, cifar_testdata,  cifar_testlabel100, cifar_testlabel20
...
def main(args):#? where is args from?
	...
	n_output_1 = 100#number of outputs
	n_output_2 = 20
	...
	with tf.variable_scope("network"):
		with contrib.framework.arg_scope(
			...
			com_result4 = combination21(conv4_1, conv4_2)
			...
			com_result5 = combination21(conv5_1, conv5_2)
	...
	with tf.Session(config = config) as sess, tf.summary.FileWriter(
			"./cifar_logs/deep-extra-layers/result", #+str(datetime.now().timestamp()),
		...
		while epoch < n_epoch:
			for _ in range(m // n_batch_size + 1):
				...
				if global_step_value % 100 == 0:
					...
					#summary_accuracy_test_value = tf.divide(summary_accuracy_test_value1 + summary_accuracy_test_value2, 2.0)
					print(summary_accuracy_test_value1, summary_accuracy_test_value2)
