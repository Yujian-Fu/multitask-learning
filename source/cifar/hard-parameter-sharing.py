...
def main(args):#? where is args from?
	...
	with tf.variable_scope("network"):
		with contrib.framework.arg_scope(
			...
			#conv2_2 = slim.layers.conv2d(pool1_1, 64,kernel_size=[3,3],scope="conv2_2")
			...
			conv5_2 = slim.layers.conv2d(conv4_1, 128, kernel_size=[3,3], scope="conv5_2")
	...
	with tf.Session(config = config) as sess, tf.summary.FileWriter(
			"./cifar_logs/hard_parameter_sharing/result", #+str(datetime.now().timestamp()),
