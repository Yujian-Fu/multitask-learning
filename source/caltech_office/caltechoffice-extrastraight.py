...
def combination31(input1, input2, input3):
	combination1 = tf.Variable(initial_value = 0.33, trainable=True, name = "combination1")
	output1 = tf.multiply(combination1, input1)

	combination2 = tf.Variable(initial_value = 0.33, trainable=True, name = "combination2")
	output2 = tf.multiply(combination2, input2)	

	combination3 = tf.Variable(initial_value = 0.33, trainable=True, name = "combination3")
	output3 = tf.multiply(combination3, input3)
...
def apply_cross_stitch(input1, input2):
    output1 = combination21(input1,input2)
    output2 = combination21(input1,input2)
...
def main(args):#? where is args from?
	...
	with tf.variable_scope("network"):
		with contrib.framework.arg_scope(
            ...
            #(?,16,16,32)->(?,16,16,32)
			com_result1 = combination21(pool1_1,pool1_2)
			...
			#(?,16,16,32)->(?,16,16,64)
			conv2_1 = slim.layers.conv2d(combination21(conv1_3, pool1_1), 64,kernel_size=[3,3],scope="conv2_1")
			conv2_2 = slim.layers.conv2d(combination21(conv1_3, pool1_2), 64,kernel_size=[3,3],scope="conv2_2")
			...
			#(?,8,8,64)->(?,8,8,64)
			com_result2 = combination21(pool2_1,pool2_2)
			conv2_3 = slim.layers.conv2d(com_result2,64, kernel_size=[3,3], scope = "conv2_3")


			#(?,8,8,64)->(?,8,8,128)
			conv3_1 = slim.layers.conv2d(combination21(pool2_1, conv2_3), 128, kernel_size=[3,3], scope="conv3_1")
			conv3_2 = slim.layers.conv2d(combination21(pool2_2, conv2_3), 128, kernel_size=[3,3], scope="conv3_2")


			#(?,8,8,128)->(?,8,8,128)
			com_result3 = combination21(conv3_1,conv3_2)
			conv3_3 = slim.layers.conv2d(com_result3, 128, kernel_size=[3,3], scope = "conv3_3")

			
			#(?,8,8,128)->(?,8,8,128)
			conv4_1 = slim.layers.conv2d(combination21(conv3_1, conv3_3), 128, kernel_size=[3,3], scope="conv4_1")
			conv4_2 = slim.layers.conv2d(combination21(conv3_2, conv3_3), 128, kernel_size=[3,3], scope="conv4_2")


			com_result4 = combination31(conv4_1, conv4_2, conv3_3)
			conv4_3 = slim.layers.conv2d(com_result4, 128, kernel_size=[3,3], scope = "conv4_3")

			#(?,8,8,128)->(?,8,8,128)
			conv5_1 = slim.layers.conv2d(combination21(conv4_1, conv4_3), 128, kernel_size=[3,3], scope="conv5_1")
			conv5_2 = slim.layers.conv2d(combination21(conv4_2, conv4_3), 128, kernel_size=[3,3], scope="conv5_2")

			com_result5 = combination31(conv5_1, conv5_2, conv4_3)
			conv5_3 = slim.layers.conv2d(com_result5, 128, kernel_size=[3,3], scope = "conv5_3")


			with tf.variable_scope("fc_6_1"):
				flatten_1 = contrib.layers.flatten(combination21(conv5_1, conv5_3))
				fc_3_1 = contrib.layers.fully_connected(flatten_1,1024)
			with tf.variable_scope("fc_6_2"):
				flatten_2 = contrib.layers.flatten(combination21(conv5_2, conv5_3))
	...
	with tf.Session(config = config) as sess, tf.summary.FileWriter(
			"./caltech_logs/deep-extra-layers/result", #+str(datetime.now().timestamp()),
...
def parse_args(argv):
	...
	parser.add_argument("--keep_prob",type=float, help="Dropout keep probability", default=0.7)
